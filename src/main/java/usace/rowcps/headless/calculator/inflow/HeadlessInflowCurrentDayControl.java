 ...
 */
public abstract class HeadlessInflowCurrentDayControl implements ICurrentDayControl
{

	private static final Logger LOGGER = Logger.getLogger(HeadlessInflowCurrentDayControl.class.getName());
	private final NavigableSet<Date> _dates = new TreeSet<>();
	
	protected final NavigableSet<Date> getDatesInMonthWithElevation(ManagerId managerId, Date date, TimeZone projectTimeZone, LocationTemplate locRef)
			throws DbException, DbConnectionException, DbIoException, DataSetTxIllegalArgumentException, DataSetException
	{
		List<Date> dates = InflowComputation.getDatesInMonth(date, projectTimeZone);
		...
		return filterDatesToLastElevDataDate(managerId, navDates, locRef);
	}

	private NavigableSet<Date> filterDatesToLastElevDataDate(ManagerId managerId, NavigableSet<Date> navDates, LocationTemplate locRef) throws DbException, DataSetException
	{
	...
	protected final NavigableSet<Date> getDatesInMonthWithElevation(ManagerId managerId, Date startDate, Date endDate, TimeZone projectTimeZone, LocationTemplate locRef)
			throws DbException, DbConnectionException, DbIoException, DataSetTxIllegalArgumentException, DataSetException
	{
		List<Date> startDates = InflowComputation.getDatesInMonth(startDate, projectTimeZone);
		List<Date> endDates = InflowComputation.getDatesInMonth(endDate, projectTimeZone);
		NavigableSet<Date> dates = new TreeSet<>(startDates);
		dates.addAll(endDates);
		dates.remove(dates.last());

		return filterDatesToLastElevDataDate(managerId, dates, locRef);
	}

	@Override
	public final void setDate(Date date, boolean fireEvents)
	{
		LOGGER.finest("Unused function");
	}
	...
	protected Date findLastElevDataDate(ManagerId manId, NavigableSet<Date> dates, LocationTemplate locRef)
			throws DbException, DbConnectionException, DbIoException, DataSetTxIllegalArgumentException, DataSetException
	{
	...
	protected final void setDates(NavigableSet<Date> dates)
	{
		_dates.clear();
		_dates.addAll(dates);
	}
