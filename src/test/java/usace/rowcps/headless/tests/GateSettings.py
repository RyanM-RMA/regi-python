...
loc = TestVariables.GATE_LOCATION
...
gateSettings.createGateSettingsOutletFromTs(office, loc,  startCal.getTime(), endCal.getTime(), "TG1", loc + "-TG1.Opening-Spillway_Gate.Const.0.0.MANUAL" )
...
gateSettings.createGateSettingsOutlet(office, loc,  startCal.getTime(), endCal.getTime(), "TG1")
...
gateSettings.createGateSettingsGroup(office, loc,  startCal.getTime(), endCal.getTime(), loc + "-TainterGate" )

# This is an example of a call that would create gate settings for every outlet in a rating group at WTYT2.
gateSettings.createGateSettings(office, loc,  startCal.getTime(), endCal.getTime() )
